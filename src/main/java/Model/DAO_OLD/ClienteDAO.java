...
public class ClienteDAO {
     ...
     * @return 
     */
    public boolean delete(Cliente cliente1){
        for (Cliente clienteLista : Banco.cliente) {
            if(idSaoIguais(clienteLista,cliente1)){
    ...
    public ArrayList<Cliente> selectFilter(String value, String filter){
    	...
    	for (Cliente cliente : Banco.cliente) {
            if(cliente.getNome().contains(value)){
    ...
    public Cliente selectPerID(int id){
    	for (Cliente cliente : Banco.cliente) {
            if(id == cliente.getId()){
                return cliente;
            }
